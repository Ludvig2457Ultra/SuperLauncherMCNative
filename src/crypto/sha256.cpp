...
void sha256_process(uint32_t state[8], const unsigned char* p) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
...
std::string sha256_hex(const void* data, size_t len) {
    ...
    size_t full = len / 64;
    for (size_t i = 0; i < full; i++) sha256_process(state, p + i * 64);

    // последний хвост + паддинг
    size_t rem = len % 64;
    ...
    if (tail_len > 56) {
        memset(tail + tail_len, 0, 128 - tail_len);
